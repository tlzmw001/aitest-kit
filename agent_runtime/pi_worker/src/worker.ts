...
import { type InitializePayload, PiSessionController, runtimeSelfTest } from "./session.ts";
...
if (process.argv.includes("--self-test")) {
  void runtimeSelfTest()
    .then((result) => process.stdout.write(`${JSON.stringify(result)}\n`))
    .catch((error) => {
      originalError(String(redact(error instanceof Error ? error.message : error)));
      process.exitCode = 1;
    });
} else {
