...
| `{suite_dir}/codegen_profile_{suite}_suite.md` | 本批用例的 case_flows/case_bodies/request_overrides |
...
- `refs/formats.md` — API Map、env 矩阵、状态影响表、profile YAML、fixture 代码、输出摘要的模板
...
4. 对新增 case 执行 Step 3 + Step 4，更新 env 矩阵、状态影响和可行性判定
...
产出：case → env 矩阵，**追加写入 api_map_{module}.md**。格式和分类规则参考 `refs/formats.md#case-环境变量矩阵`。
...
输出：完整 `codegen_profile_{module}.md`。路线选择记录附在 profile 末尾或输出摘要中。
