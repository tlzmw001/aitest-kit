...
| `codegen_profile_{module}.md` | module_type、共享 assertion_rules、默认 fixture/object、L1 稳定能力 |
| `{suite_dir}/aitest_suite.yaml` | 用例 suite 归属：module、suite、case_files、suite profile |
| `{suite_dir}/codegen_profile_{suite}_suite.md` | 本批用例的 variables、case_flows/case_bodies/request_overrides |
| `api_map_{module}.md` | API 面 + env 契约 + 可行性判定（scaffold 过程产物，保留供 review） |

模块级文件路径相对于 `test_workspace/tests/fixtures/`。suite 级文件跟随具体用例目录。
...
## Step 0：模式选择
确认：**模块名** + **模式**（scaffold-module / scaffold-suite / incremental）。
...
```text
{suite_dir}/aitest_suite.yaml
{suite_dir}/codegen_profile_{suite}_suite.md
```
...
2. 读取现有 fixture、module profile、suite profile、helpers、conftest 和新增 cases
...
非幂等且无 cleanup API 的 case 默认标为可行性存疑。可行性判定是 Step 7 的显式输入——skip_list 中的 case 不生成 case_flow/case_body。

**用户确认**：非幂等 case 是否可接受、cleanup 策略、是否需要测试专用资源。用户可以将可行性存疑的 case 移入可执行或确认保持 skipped。

## Step 5：生成 fixture + conftest 接线

基于已确认的 Step 2（Client 签名 + auth 标注）和 api_map 中的 variables/env 矩阵生成 `fixtures/{module}.py`。
...
conftest 接线（原子步骤）：生成 fixture 文件后，立即检查 `test_workspace/tests/conftest.py` 注册并运行 collect 验证。fixture 生成和 conftest 接线不分开。
...
输出：scaffold-module 模式生成完整 `codegen_profile_{module}.md`；scaffold-suite 模式生成 `{suite_dir}/codegen_profile_{suite}_suite.md`。suite profile 优先承载 `variables` 和本批 case 的 `case_flows/case_bodies/request_overrides`。路线选择记录附在 profile 末尾或输出摘要中。
