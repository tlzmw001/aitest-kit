...
HTTP 基础请求体必须是“真实默认请求”，不是模板。即使某个模块后来全部走 `case_flow`，Markdown 里的 JSON 也不能写 `"{{user_id}}"` 这类占位符；parser 会把它作为 E001 硬错误交给 profile gate 阻断。case 级差异应该放到 `request_overrides`、fixture helper 或 `case_flows` 中。
...
| Markdown JSON 中出现 `{{var}}` | 基础请求体被写成模板而不是真实默认请求 | 改成合法默认值，差异放到 profile/fixture/case_flow |
...
新项目 fixture 还有一个容易被 `case_flow` 掩盖的坑：不要在缺少项目专属 base URL 时回退到老项目的 `http_base_url`。正确做法是读取项目自己的环境变量，例如 `DISCOUNT_SYSTEM_BASE_URL`；如果没配置，直接 `pytest.fail(...)`。这样失败会指向“环境未配置”，而不是把请求偷偷打到另一个服务。
