...
- **优先级**：P2
- **场景变量**：
  - 前置操作：通过 AB 服务创建实验 `ab_boundary_left`，策略 `left_hit` 的 `hash_range=[H,H+1]`
...
- **优先级**：P2
- **场景变量**：
  - 前置操作：通过 AB 服务创建实验 `ab_boundary_right`，策略 `right_miss` 的 `hash_range=[0,H]`
  - 前置操作：选择 `md5(user_id)%100 == H` 的 `user_id`
  - 前置操作：将 `scene_id=1001` 映射到该实验
...
- **场景变量**：
  - 协议：HTTP
  - 接口调用：`PUT /api/v1/ab/whitelist/u_ab_invalid_white`，body 为 `{"strategy_map":{"coarse_rank_exp_game":"not_exists_strategy"}}`
  - 请求覆盖：HTTP 请求 `user_id="u_ab_invalid_white"`、`reqId="req-ab-012"`
- **断言**：`exp["coarse_rank_exp_game"] != "not_exists_strategy"`； AB 服务日志包含 `ab_sdk whitelist invalid`
- **标记**：`[manual]`
...
- **场景变量**：
  - 协议：HTTP
  - 环境覆盖：使用本地 SDK 模式启动主服务，环境变量 `AB_SDK_WHITELIST_JSON="{invalid json"`
  - 请求覆盖：HTTP 请求 `user_id="u_ab_bad_env"`
- **断言**：请求不因白名单解析失败中断；`response.body.code == 0`；`exp` 来自 hash 分流或为空
- **标记**：`[!可行性存疑: 需要测试环境支持本地 SDK 模式启动主服务]`
...
- **场景变量**：
  - 协议：HTTP
  - 请求覆盖：AB 服务端点接受连接但 `/api/v1/ab/evaluate` 响应超过 SDK timeout
  - 请求覆盖：HTTP 请求 `user_id="u_ab_timeout"`
- **断言**：HTTP 推荐接口返回 `500`
- **标记**：`[!可行性存疑: 需要测试环境提供慢响应 AB 服务]`

### TC-AB-015：主服务早于 AB 服务启动时首个实验请求失败
- **优先级**：P2 / 异常
- **场景变量**：
  - 协议：HTTP
  - 环境覆盖：先启动主服务，不启动 AB 服务
  - 请求覆盖：发送 `external=0` 的 HTTP 推荐请求
  - 请求覆盖：随后启动 AB 服务再次发送同请求
- **断言**：AB 服务未启动时返回 `500`；AB 服务启动后同请求返回 `code == 0`
- **标记**：`[!可行性存疑: 需要测试环境允许控制 AB 服务启动顺序并在同一用例内重试请求]`
