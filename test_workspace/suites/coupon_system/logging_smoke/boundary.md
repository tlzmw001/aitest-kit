...
```json
{
  "user_id": "u_log_default",
  "scene_name": "game",
  "device": "mobile",
  "policy_id": "",
  "external": 0,
  "reqId": "req_log_default",
  "score_threshold": 0.0,
...
- **场景变量**：
  - 协议：HTTP
  - 环境覆盖：以默认 `python -m coupon_system.main` 启动服务，不额外调用 `logging.basicConfig(level=logging.INFO)`
  - 请求覆盖：HTTP 请求 `reqId="req-log-009"`
- **断言**：业务请求返回 `code == 0`；stdout/stderr 中找不到 `recommend request: reqId=req-log-009`；详见 mismatch.md
- **标记**：`[!可行性存疑: 需要测试环境能区分 uvicorn 日志和业务 logger 输出]`

### TC-LOG-010：显式配置 INFO 后业务日志可见
- **优先级**：P2
- **场景变量**：
  - 协议：HTTP
  - 环境覆盖：测试启动入口显式配置 `logging.basicConfig(level=logging.INFO)` 后启动服务
  - 请求覆盖：HTTP 请求 `reqId="req-log-010"`
...
- **场景变量**：
  - 协议：HTTP
  - 前置操作：测试进程安装一个会在 `emit()` 抛异常的 logging handler
  - 请求覆盖：HTTP 请求 `reqId="req-log-011"`
- **断言**：业务响应仍为 `code == 0`；logging 内部 `handleError` 处理异常，无业务错误码
- **标记**：`[!可行性存疑: 需要测试环境能注入 logging handler]`

### TC-LOG-012：日志中的 item_ids 为空字符串时仍输出字段名
- **优先级**：P2 / 异常
- **场景变量**：
  - 前置操作：无法通过接口发送空 items（会被参数校验拦截）
  - 前置操作：使用专项组件测试构造空 items 调用日志代码路径
- **断言**：日志仍包含 `item_ids=` 字段；详见 mismatch.md
- **标记**：`[!可行性存疑: 黑盒接口无法覆盖，需组件级专项验证]`
