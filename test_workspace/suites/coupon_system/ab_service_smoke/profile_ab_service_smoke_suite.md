...
case_flows:
  TC-ABS-012:
    steps:
    - call: harness.isolated_experiment_persists
      save_as: result
  ...
  TC-ABS-018:
    steps:
    - call: harness.isolated_whitelist_persists
      save_as: result
  ...
  TC-ABS-026:
    steps:
    - call: harness.missing_experiments_file_is_created
      save_as: result
  ...
  TC-ABS-027:
    steps:
    - call: harness.malformed_whitelist_falls_back_empty
      save_as: result
    - assert: assert result['status'] == 200
    - assert: assert result['body'] == {}
  ...
  TC-ABS-028:
    steps:
    - call: harness.bad_hash_range_still_evaluates
      save_as: result
    - assert: assert result['status'] == 200
    - assert: assert result['strategy_id'] == 's_bad'
  TC-ABS-029:
    steps:
    - call: harness.bad_params_fall_back_empty
      save_as: result
    - assert: assert result['status'] == 200
    - assert: assert result['params'] == {}
  TC-ABS-033:
    steps:
    - call: harness.import_works_from_other_cwd
      save_as: result
    - assert: assert result['returncode'] == 0, result['stderr']
    - assert: assert 'ok ab_experiment_sdk.service' in result['stdout']
  TC-ABS-034:
    steps:
    - call: harness.import_has_no_default_file_side_effect
      save_as: result
    - assert: assert result['returncode'] == 0, result['stderr']
    - assert: assert 'exists False' in result['stdout']
  TC-ABS-035:
    steps:
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_game, exp_cal]
    - call: harness.remote_sdk_evaluate_whitelist
      save_as: result
  ...
  TC-ABS-036:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_game, exp_cal]
    - call: harness.remote_sdk_set_user_whitelist
      save_as: result
  ...
  TC-ABS-037:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_game, exp_cal]
    - call: harness.remote_sdk_clear_user_whitelist
      save_as: result
  ...
  TC-ABS-038:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_game, exp_cal]
    - call: harness.remote_sdk_replace_whitelist
      save_as: result
  ...
  TC-ABS-039:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_game, exp_cal]
    - call: harness.remote_sdk_clear_all_whitelist
      save_as: result
    - assert: assert result['whitelist'] == {}
  TC-ABS-040:
    steps:
    - call: harness.remote_sdk_raises_on_http_error
      save_as: result
  ...
  TC-ABS-003:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_ab_basic]
    - call: harness.prepare_user_whitelist
      kwargs:
        user_id: u_abs_white
        strategy_map:
          exp_ab_basic: s_b
  ...
  TC-ABS-004:
    ...
    - call: harness.prepare_standard_experiments
      kwargs:
        names: [exp_ab_basic, exp_ab_extra]
    - call: harness.post
      args:
      - /api/v1/ab/evaluate
      - user_id: u_abs_hash_0
  ...
  TC-ABS-009:
    ...
    - assign: payload
      expr: '{''name'': ''exp_abs_create'', ''strategies'': [{''id'': ''s1'', ''hash_range'': [0, 100], ''params'': {''k'':
        ''v''}}]}'
    - call: harness.snapshot_experiment
      args:
      - exp_abs_create
    - call: harness.post
      args:
  ...
  TC-ABS-010:
    steps:
    - call: harness.upsert_experiment
      args:
      - name: exp_abs_update
        strategies:
        - id: s_old
  ...
  TC-ABS-014:
    steps:
    - call: harness.snapshot_whitelist
    - call: harness.put
      args:
  ...
  TC-ABS-017:
    steps:
    - call: harness.prepare_whitelist
      kwargs:
        whitelist:
          user_b:
            exp_game: game_on
