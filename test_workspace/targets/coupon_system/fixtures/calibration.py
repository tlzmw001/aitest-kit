...
@pytest.fixture
def ab_base_url() -> str:
    return os.environ.get("AB_SERVICE_URL", "http://127.0.0.1:8100")


@pytest.fixture
def grpc_target() -> str:
    return os.environ.get("GRPC_TARGET", "127.0.0.1:50051")


def _write_json_rules(target_dir: Path, files: dict) -> None:
    target_dir.mkdir(parents=True, exist_ok=True)
    for fname, rules in files.items():
        (target_dir / fname).write_text(json.dumps(rules), encoding="utf-8")
...
def setup_calibration(http_base_url, ab_base_url, tmp_path):
    ...
    if _original_experiment is not None:
        try:
            ...
            warnings.warn(
                f"failed to restore calibration experiment {_CAL_EXPERIMENT_NAME}: {exc}",
                RuntimeWarning,
                stacklevel=1,
            )
    for uid in _whitelist_users:
        try:
            ...
            warnings.warn(
                f"failed to delete AB whitelist for {uid}: {exc}",
                RuntimeWarning,
                stacklevel=1,
            )
