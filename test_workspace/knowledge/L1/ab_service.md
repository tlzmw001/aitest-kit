...
- [test_workspace/cases/ab_service/business.md] AB 服务业务+异常用例
  - 已覆盖：健康检查、hash/白名单评估、experiment_names 三种行为、实验列表/详情查询、实验 CRUD、实验持久化、白名单 CRUD、白名单持久化、404/409/400、删除不存在用户白名单静默成功
- [test_workspace/cases/ab_service/boundary.md] AB 服务边界用例
  - 已覆盖：hash_range 重叠行为（first-match）、空策略实验、不存在实验名跳过、实验配置文件不存在自动创建、白名单文件损坏容错、策略格式异常回退、Pydantic 422 校验、service 独立导入、导入无 cwd 副作用、Remote SDK evaluate/白名单管理/服务端 500
- [cases/old-cases/ab_remote_client.md] 远程 SDK
  - 已覆盖：远程 evaluate 端到端、SDK 白名单 CRUD、远程服务 500 异常（已迁移到 test_workspace/cases/ab_service/boundary.md）
