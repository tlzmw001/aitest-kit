...
class TestLoggingBoundary:
    ...
    def test_tc_log_010(self, setup_logging):
        ...
        __tc_meta__ = {
            ...
            "module": "logging",
            "category": "boundary",
            "source": "test_workspace/cases/logging/boundary.md",
...
__codegen_skipped__ = [{"tc_id": "TC-LOG-009", "module": "logging", "category": "boundary", "source": "test_workspace/cases/logging/boundary.md", "title": "未配置 root logger 时 INFO 业务日志不可见", "priority": "P2 / 异常", "markers": ["`[!可行性存疑: 需要测试环境能区分 uvicorn 日志和业务 logger 输出]`"], "reason": "`[!可行性存疑: 需要测试环境能区分 uvicorn 日志和业务 logger 输出]`"}, {"tc_id": "TC-LOG-011", "module": "logging", "category": "boundary", "source": "test_workspace/cases/logging/boundary.md", "title": "日志 handler 写入失败不影响业务响应", "priority": "P2 / 异常", "markers": ["`[!可行性存疑: 需要测试环境能注入 logging handler]`"], "reason": "`[!可行性存疑: 需要测试环境能注入 logging handler]`"}, {"tc_id": "TC-LOG-012", "module": "logging", "category": "boundary", "source": "test_workspace/cases/logging/boundary.md", "title": "日志中的 item_ids 为空字符串时仍输出字段名", "priority": "P2 / 异常", "markers": ["`[!可行性存疑: 黑盒接口无法覆盖，需组件级专项验证]`"], "reason": "`[!可行性存疑: 黑盒接口无法覆盖，需组件级专项验证]`"}]
