...
- plugin v0 prototype 阶段：
  ...
  - 先实现 2 到 3 个入口 skill：onboard、generate tests、run tests。
  - 通过本地 workspace 验证。
  - 当前 prototype 路径：`plugins/aitest-kit/`，包含 `plugin.json`、README 和三个入口 skill，不进入 PyPI 包或 workspace template。
