...
| `test_workspace/targets/{target}/modules/{module}.yaml` | module 归属、fixture/profile、registered_suites |
...
新结构下，模块级 fixture/profile/helper 归属于 `test_workspace/targets/{target}/`；suite 级文件跟随具体用例目录。legacy `test_workspace/tests/fixtures/` 只作为旧 workspace 兼容路径，不再作为新 scaffold 的默认落点。
...
registry 接线（原子步骤）：生成 fixture 文件后，立即检查 `test_workspace/targets/{target}/modules/{module}.yaml` 是否声明 `fixture.file/default_fixture`。suite generated pytest 会从 module.yaml 自动注入 fixture import。legacy 模块模式才需要检查 `test_workspace/tests/conftest.py` 的 `pytest_plugins` 注册。
...
输出：scaffold-module 模式生成完整 `profile_{module}.md`；scaffold-suite 模式生成 `{suite_dir}/profile_{suite}_suite.md`。suite profile 优先承载 `variables` 和本批 case 的 `case_flows/case_bodies/request_overrides`。路线选择记录附在 profile 末尾或输出摘要中。
...
允许：读 docs/knowledge/cases/、经确认读 API 声明层、读其他模块 fixture/profile、读 project_config、创建/修改 target 下的 fixture/profile/helper/module.yaml 和 api_map、运行验证命令。legacy 模块模式才允许更新 conftest.py 的 pytest_plugins 列表。
