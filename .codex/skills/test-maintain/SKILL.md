...
- `refs/routing.md` — 症状映射、codegen 模式速查、验证命令、废弃影响面模板
...
| 断裂层 | 路由 | 交接信息 |
|--------|------|----------|
| knowledge | `knowledge-build` | 模块名、缺口描述、关联文档 |
| cases | `test-design`（新增）/ `test-fix`（修正） | 模块+知识库引用 / case_id+错误描述 |
| scaffold | `test-scaffold` | target、模块、缺口清单 |
| codegen | `test-codegen` | suite.yaml 路径 |
| execution | `aitest run` | 无需路由 skill |
| emitter | `emitter-build` | 已验证 pytest、可沉淀模式 |
...
底层 skill 完成后，重跑 Step 2 的诊断命令，确认断裂层已修复。

发现新的断裂层（如修了 scaffold 后 codegen 需要重跑）时，继续路由下一层。

验证命令序列见 `refs/routing.md`。
...
本 skill 不维护静态的 skill 列表或 CLI 命令表。运行时发现可用能力：

- **可用 skill**：读取当前对话中 system prompt 列出的 available skills
- **CLI 命令**：`aitest --help` 和 `aitest <subcommand> --help`
- **项目结构**：`aitest_config/aitest.yaml`、`test_workspace/targets/` 发现已注册 target/module/suite
- **工作流**：读取 `CLAUDE.md` 了解完整飞轮流程

遇到无法路由的请求时，说明缺少对应能力，建议用户决定是否新增 skill 或走人工流程。不强行套用现有 skill。
