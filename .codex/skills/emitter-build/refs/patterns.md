...
| `promote_to_case_flow` | 多步骤流程稳定，差异主要是参数、保存变量和期望值 | suite profile |
| `promote_to_helper` | 多个 body/flow 重复 Python 逻辑 | fixture/helper |
| `promote_to_default_template` | 可退回 default_http/default_grpc，且默认模板真实适配 | suite profile request_overrides 或断言规则 |
| `promote_to_named_template` | 需要生成受控 if/elif/else 代码，且跨模块稳定 | emitter/renderer |
| `keep_case_body` | 少见、复杂、并发、进程、mock、文件生命周期等暂不晋升 | suite profile |
...
3. fixture/helper
4. keep_case_body
5. emitter/renderer 规则最后考虑

不要因为 default_http/default_grpc 已存在就强行退回默认模板；新项目、多端点项目和复杂 API 通常应优先使用 fixture Client + case_flow。
...
4. 晋升后比原 case_body 更可读、更可解释、更容易校验。
5. 晋升后 `--validate-profile`、`--check`、collect/run 能通过。

不满足时保留 case_body，并在输出摘要中说明原因。

以下情况通常保留 case_body，或先抽 helper 后再评估：
...
- generated pytest 默认位于 `test_workspace/generated/{target}/`。
- `promotion_report.md/json` 用于解释和工具消费。
- `promotion_patch.md/diff` 是 review-only 草案，默认不自动修改 profile。

## 人工晋升闭环

1. 确认对应 generated pytest 已验证通过。
2. 运行 `--analyze-promotion --write-report` 生成候选依据。
3. 必要时运行 `--suggest-promotion-patch` 生成 review-only 草案。
4. 人工 review 后修改 suite profile、module profile、fixture/helper 或 `aitest.yaml`。
5. 如果把 body 晋升为 flow，同步删除同 case_id 的旧 `case_bodies`。
6. 运行 `--validate-profile`、`--dump-ir`、`--check` 和 pytest collect/run。
7. 用 `--health-report --write-report` 确认 case_body/UNPARSED 数量下降或至少不恶化。
...
- 复杂循环断言不要直接塞进 flow。优先抽 helper，例如 `client.assert_all_publish_status(resp, 0)`。
- 如果循环/分支逻辑本身就是测试主体，保留 case_body。
