...
```
Markdown 用例 → parser（结构化提取）→ emitter（代码生成）→ AI 补写 UNPARSED → pytest .py
```

1. **parser** 确定性地将 Markdown 解析为 `SharedConfig` + `TestCase` 列表
2. **emitter** 用断言规则模板将结构化数据生成 `.py` 文件
3. 无法模板匹配的断言输出为 `# UNPARSED ASSERTION:`，由 AI 补写
4. 生成后自动 `ast.parse` 语法检查
...
| 测试客户端 | httpx / grpcio |
| 测试框架 | pytest |
| 代码生成 | aitest_kit（parser + emitter）|
