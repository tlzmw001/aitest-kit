...
- `test_workspace/cases/`
  Markdown 测试用例目录，按模块组织，是 codegen 的核心输入。

- `test_workspace/casesuites/`
  可选的独立用例批次目录，通过 `aitest_suite.yaml` 绑定到某个模块。

- `test_workspace/tests/fixtures/`
  模块级 fixture 与 `codegen_profile_{module}.md`。
...
3. 用例设计：在 `test_workspace/cases/{module}/` 编写模块用例；或在 suite 目录编写 L2/迭代批次用例。
4. profile/fixture：在 `test_workspace/tests/fixtures/` 增加模块 fixture 和 module profile；suite 用例在用例目录旁维护 `_suite` profile。
...
- `codegen_profile_{module}.md` 与模块 fixture 同目录；`codegen_profile_{suite}_suite.md` 跟随具体用例 suite。
