...
排查失败时可加 `--capture`，当前 run 目录会写入一个 `capture.jsonl`。默认只自动捕获框架生成的 HTTP 用例；自定义 fixture、gRPC 或 SDK 调用可以手动调用 `aitest_kit.helpers.capture.capture_io()`，在 generated 测试函数体内调用时会自动归因到当前 case，显式传入 `case_id` 仍然有效。pytest fixture setup/teardown 阶段不在该 context 内。capture 不自动脱敏，敏感字段应先在 fixture 中处理。
