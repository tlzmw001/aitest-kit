...
- `[?行为未定义: ...]` — 文档未说明某条件下的系统行为（如"资源列表为空时的行为"），需补充文档或找产品/研发确认
- `[?值未明确: ...]` — 具体字段名、加密方式、阈值等文档未给出（如"业务特征字段名未列出"），需补充文档、配置或接口定义
- `[?可观测性缺失: ...]` — 模块缺少日志/指标/健康检查的描述，需审计实际可观测覆盖度

补全时建议按目标系统的数据流顺序逐模块处理（入口校验→鉴权/路由→业务处理→外部依赖→状态变更→响应），上下文连贯性更好，更容易发现跨模块问题。

## 错误场景标注规范

错误场景段落中，区分两类：

- **设计行为**：系统有意为之的降级/兜底（如"资源列表为空→返回空列表"），直接描述行为
- **`[!风险]`**：系统未处理的异常路径（如"外部缓存/依赖连接异常未捕获，会导致整个请求失败"），用 `[!风险]` 标记，这些是高价值测试点

## 可观测状态段落要求

每个 L1 模块必须填写"可观测状态"段，包含：

- 已有的可观测手段（日志级别+内容、存储 Key、API 端点、指标等）
- **盲区**：关键路径上缺少日志/指标的地方（如"目录不存在时无日志"、"限流触发无日志"）
- 已定义但未使用的错误码/常量（如"RESOURCE_NOT_FOUND 已定义但未在文档行为中出现"），这类信息对测试设计有价值
