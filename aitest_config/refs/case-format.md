# 用例格式：共享配置 + 精简用例

> 本文档定义测试用例的 Markdown 输出格式。
> 引用方：test-design skill（生成用例）、codegen（解析 MD 生成 pytest）

## 设计原则

- `suite.yaml` 声明一批用例属于哪个 target/module/suite，以及包含哪些 Markdown case files
- Markdown case file 只表达测试意图；`business.md`、`boundary.md` 是推荐命名，不是强制结构
- 每个用例文件顶部建议定义**共享配置**，包含所有用例共用的部分
- 每条用例只写与共享配置不同的**场景变量**和**断言**
- 目标：人类 review 时一眼看到"这条用例测什么"，不被重复信息干扰
- Markdown 不负责执行接线；默认请求差异优先写入 suite profile 的 `requests.<case_id>.patches`，简单字段覆盖可用 `overrides`；多步骤流程写入 suite profile 的 `case_flows`，复杂控制逻辑写入 suite profile 的 `case_bodies` 或沉淀到 fixture/helper

## 文件结构

```markdown
# {模块名} 业务测试用例

> 生成方式：test-design skill
> 关联知识库：L1/xxx
> 生成日期：YYYY-MM-DD

---

## 共享配置

**接口**：`POST /api/v1/recommend` / `gRPC coupon.CouponService/Recommend`

**基础请求体（JSON）**：
（完整、合法的 JSON。也兼容 `基础请求体` / `基础请求体（HTTP）` / `基础请求体（gRPC）` 写法；变化字段写合法默认值，不使用 `{{placeholder}}`；默认请求路线和 `case_flow` 的 `request_ref` 都通过 suite profile 的 `requests` 替换字段；`基础请求体（gRPC）` 只作为兼容标签，不代表存在 `default_grpc`）

**标准前置**：
- 所有用例共用的环境准备步骤（路由配置、库存初始化等）

**通用断言**：`response.code == 0`

**变量定义**：
- `s` = `response.results[0].score`（打分模型返回的原始分数，不可预知）
- `cal` = `response.results[0].calibrated_score`
- `clamp(x, 0, 1)` = `max(0, min(1, x))`

---

## 一、{场景分组名}

### TC-XXX-001：{一句话描述}
- **优先级**：P1
- **场景变量**：
  - 协议：HTTP
  - 请求覆盖：`{"user_id": "u_xxx_001"}`
  - 前置操作：校准文件 `linear/1.json` 内容 `{"conditions":{"device":"mobile"}, "k":1.5, "b":0.1}`
- **断言**：`cal == clamp(1.5 * s + 0.1, 0, 1)`

### TC-XXX-002：{一句话描述}
- **优先级**：P1
- **场景变量**：请求覆盖：`{"user_id": "u_xxx_002"}`
- **断言**：`cal == s`（校准跳过）
- **标记**：`[manual]`

---

## 覆盖变更

| 知识库文档 | 新增覆盖 | 仍未覆盖 |
|-----------|---------|---------|
| L1/xxx | 维度 A、维度 B | 维度 C |
```

## 共享配置区字段说明

| 字段 | 必填 | 说明 |
|------|------|------|
| 接口 | 是 | 列出模块暴露的所有接口类型（HTTP endpoint + gRPC method） |
| 基础请求体 | 默认 HTTP 路线和 `case_flow.request_ref` 必填 | 完整的 JSON 请求体；可写 `基础请求体` / `基础请求体（JSON）` / `基础请求体（HTTP）` / `基础请求体（gRPC）`，不允许 `{{var}}` 占位符。所有标签都会统一解析为 JSON 基础请求体；`基础请求体（gRPC）` 只是兼容标签，不代表存在 `default_grpc`。不使用 `request_ref` 的 gRPC、SDK、多端点或全部由 `case_flows/case_bodies` 覆盖的用例不依赖默认请求体 |
| 标准前置 | 是 | 所有用例共用的环境准备（路由表、库存、基础配置等） |
| 通用断言 | 是 | 每条用例都要检查的断言 |
| 变量定义 | 按需 | 关系断言中的变量缩写，减少每条用例的重复 |

## 每条用例字段说明

| 字段 | 必填 | 说明 |
|------|------|------|
| 优先级 | 是 | P0/P1/P2，异常类型追加标注（如 `P1 / 异常`） |
| 场景变量 | 是 | 与共享配置的差异，必须写成 `key：value` 条目列表（见下方书写约束）。注意：这里的“请求覆盖”用于设计 review；默认请求路线的真实请求覆盖以 suite profile 的 `requests` 为准 |
| 断言 | 是 | 可程序化的断言表达式，引用变量定义中的缩写 |
| 标记 | 否 | `[manual]`、`[!可行性存疑: 原因]` 等标记，独立一行，不内联到其他字段 |

## 场景变量书写约束

场景变量必须写成 `key：value` 条目列表，禁止写成自然语言段落。每个条目一行，key 用中文冒号分隔。

单条目可简写为行内格式：

```markdown
- **场景变量**：请求覆盖：`{"scene_name": "ad", "device": "pc"}`
```

多条目换行：

```markdown
- **场景变量**：
  - 请求覆盖：`{"scene_name": "ad", "device": "pc"}`
  - 前置操作：`PUT /api/v1/ab/whitelist/u_xxx {"exp": "strategy_off"}`
```

key 名称不做全局限定，由各项目按实际需要自行约定，保持同一模块内一致即可。

## 执行接线边界

- Markdown 只描述场景、输入差异、前置意图和断言意图。
- 默认请求差异写入 suite profile 的 `requests.<case_id>.patches`；简单字段覆盖可用 `overrides`，复杂 list/dict/delete/env/value 注入优先 `patches`。
- 多步骤流程写入 suite profile 的 `case_flows`。
- 循环、条件分支、mock、文件生命周期、跨进程等复杂逻辑写入 suite profile 的 `case_bodies`，或沉淀到 fixture/helper。
- 不要把 token、密钥、真实账号值写进 Markdown；只写环境变量名、资源别名或测试语义，具体值由 env/runtime variables 提供。
- 区分可控输入与系统中间产物：请求参数、Redis 数据、配置文件、实验参数属于可控输入；打分分数、排序位次、校准后分数等 pipeline 计算结果属于中间产物，不能在前置条件或场景变量中假设其具体值。

## 标记字段

当用例存在特殊状态时，使用独立的"标记"字段，不要把标记内联到场景变量或断言中：

```markdown
### TC-XXX-009：某场景描述
- **优先级**：P2 / 异常
- **场景变量**：...
- **断言**：...
- **标记**：`[manual]`、`[!可行性存疑: 需要测试环境能区分 X 和 Y]`
```

已定义标记：
- `[manual]` — 无法自动化执行，需人工验证
- `[!可行性存疑: 原因]` — 用例设计合理但执行可行性待确认

## 旧版完整格式（兼容）

已有用例可保留以下格式，不强制迁移：

```markdown
### TC-{模块}-{序号}：{一句话描述}
- **关联**：L1/xxx 或 L2/xxxx
- **优先级**：P0 / P1 / P2
- **类型**：业务 / 边界 / 异常
- **前置条件**：环境状态、数据准备
- **输入**：完整请求体（JSON 格式）
- **测试步骤**：
  1. 动作一
  2. 动作二
- **预期结果**：可断言的具体值
```
